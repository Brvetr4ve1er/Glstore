...
| [api/routes/customer_account.py](api/routes/customer_account.py) | `/account/orders` | Signed-in customer's own orders (list + detail), looked up under every legacy phone key so COD orders filed as `0555…`/`+213555…`/`555…` all appear. Host-scoped; 404 for anything not theirs |
...
| [api/routes/financing_workflow.py](api/routes/financing_workflow.py) | `/financing/...` | Signed-in customer: profile, document upload/delete, submit (DRAFT → SUBMITTED). Uploads go through the API: size-capped, type sniffed from bytes, stored in a PRIVATE bucket via [api/services/documents/](api/services/documents/__init__.py) — 503 until configured |
| [api/routes/financing_review.py](api/routes/financing_review.py) | `/financing/review/...` | Admin review queue: open file, examine documents, approve/reject/sign; required-documents config. Status only changes via `repo.transition()`; the machine is enforced by a trigger (migration 013). Documents streamed as attachments, reads logged by admin id |
| [api/routes/partners.py](api/routes/partners.py) | `/partners/apply`, `/partner-applications`, `/partners` | Point-of-sale applications (public, one stable answer — no "already applied" leak), admin queue, approve = partner + first location in one transaction (migration 014). No partner ships |
